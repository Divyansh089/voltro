...
export default function CategoryListing({ category }: { category: string }) {
  ...
  return (
    <>
      <Head>
        <title>{`${cat?.name || category} — Voltra`}</title>
      </Head>
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-5 md:grid-cols-3">
                {filtered.map((p) => (
                  <div
                    key={p.id}
                    className="glass group relative flex flex-col p-4 transition hover:-translate-y-0.5"
                  >
                    <button
                    ...
                    <Link
                      href={`/product/${p.id}`}
                      className="mt-2 font-display text-base font-semibold text-ink hover:underline"
                    >
                      {p.name}
                    </Link>
